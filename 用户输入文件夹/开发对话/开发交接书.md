...
- **影响**:
  ...
  - 无法区分不同视频生成任务的结果
- **原因**: 事件系统未正确验证 `connectedSourceId`
- **修复方案**:
  1. 检查 TaskResultNode 的事件监听逻辑
  2. 确保 `data.connectedSourceId === event.detail.sourceNodeId` 验证正确
  3. 考虑使用更具体的事件名（如 `video-task-created-${nodeId}`）
- **影响文件**:
...
- **验证方法**: 检查 API 请求体中是否包含 `duration` 参数
- **可能原因**:
  1. 前端未正确传递 `totalDuration` 到 API
  2. 后端 sora2-client.js 未正确处理 `duration` 参数
  3. API 不支持 duration 参数（需要验证文档）
- **影响文件**:
  - `src/client/src/nodes/process/StoryboardNode.jsx`
  - `src/server/sora2-client.js`
- **优先级**: 高
