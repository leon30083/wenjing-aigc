...
GET /api/history/:taskId            # 获取单条记录
DELETE /api/history/:taskId         # 删除记录
...
#### 问题1: 历史记录加载后 TaskResultNode 显示错误视频 ⚠️ 2026-01-01
- **现象**: 从历史记录加载工作流后，TaskResultNode 显示的视频不正确
- **日志表现**:
  - 修复前: `[TaskResultNode] Skipping restore (new task in progress)` - 被拦截
  - 修复后: `[TaskResultNode] Loaded from history (flagged), restoring state` - 恢复成功
  - 但用户反馈: 问题仍然存在，显示的视频与历史记录不匹配
- **已尝试修复**:
  1. **修复1**: 调整 useEffect 1 检查顺序（优先检查 `_isCompletedFromHistory`）
     - 代码位置: `src/client/src/nodes/output/TaskResultNode.jsx` 第 42-93 行
     - 效果: 日志显示恢复成功，但视频仍不正确
  2. **修复2**: 加载历史记录时覆盖 TaskResultNode 数据（错误34的短期方案）
     - 代码位置: `src/client/src/App.jsx` `handleLoadWorkflowFromHistory` 函数
     - 状态: 可能还未实现或实现不完整
- **可能原因**:
  1. **快照时机问题**: VideoGenerateNode 捕获快照时，TaskResultNode 的数据还未同步（错误34）
  2. **事件系统时序**: `video-task-created` 事件派发后，TaskResultNode 接收并更新，但快照已捕获旧数据
  3. **useEffect 异步执行**: useState 更新是异步的，useEffect 在渲染后执行
  4. **轮询覆盖**: 历史记录加载后，轮询可能仍在运行并覆盖数据
- **下一步调试**:
  1. 检查 `handleLoadWorkflowFromHistory` 是否正确覆盖 TaskResultNode 数据
  2. 检查加载历史记录后，轮询是否仍在运行
  3. 检查 `video-task-created` 事件监听器是否正确移除
  4. 在历史记录加载时，强制停止所有轮询
- **相关错误模式**:
  - 错误34: 工作流快照时机问题
  - 错误35: 历史记录加载 useEffect 检查顺序问题
- **文档更新**:
  - ✅ SKILL.md: 添加错误35
  - ✅ code.md: 添加错误35
