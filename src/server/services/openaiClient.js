...
- 不能描述角色外观（Sora2会使用角色真实外观）`;
    ...
    if (style === 'picture-book') {
      // ⭐ 根据是否有角色上下文选择不同的示例
      const exampleSection = (context.characters && context.characters.length > 0)
        ? `✅ 必须包含角色引用的示例：
"卡通绘本风格的视频。@${context.characters[0].username} 在阳光明媚的花园里欢快地追逐蝴蝶，跳跃着探索每一处角落。画面色彩明亮饱和，充满童趣，动作夸张且富有弹性，背景细节丰富，光影效果梦幻，适合10秒的视频时长。"`
        : `✅ 示例：
...
- 包含所有必要信息（风格、角色、场景、氛围、动画、时长）`;
    ...
    if (style === 'cinematic') {
      ...
      const exampleSection = (context.characters && context.characters.length > 0)
        ? `✅ 必须包含角色引用的示例：
"电影风格的视频。@${context.characters[0].username} 孤独地走在夕阳西下的城市街道人行道上。画面采用广角镜头拍摄，浅景深突出主体，温暖的黄金时刻光线从侧面照射，营造出怀旧而忧伤的氛围。色彩倾向为暖色调，情绪氛围温馨而略带忧郁，适合10秒的视频时长。"`
        : `✅ 示例：
...
- 包含所有必要信息（风格、场景、摄影、氛围、时长）`;
    ...
    if (style === 'documentary') {
      ...
      const exampleSection = (context.characters && context.characters.length > 0)
        ? `✅ 必须包含角色引用的示例：
"纪录片风格的视频。@${context.characters[0].username} 在建筑工地上工作，真实记录日常工作场景。采用旁观者视角，自然光线照射，构图平衡稳重，展现装载机的工作状态和工地环境，适合10秒的视频时长。"`
        : `✅ 示例：
...
- 包含所有必要信息（风格、场景、拍摄、信息、时长）`;
    ...
    if (style === 'animation') {
      ...
      const exampleSection = (context.characters && context.characters.length > 0)
        ? `✅ 必须包含角色引用的示例：
"动画风格的视频。@${context.characters[0].username} 在阳光明媚的花园里欢快地追逐蝴蝶，跳跃着探索每一处角落。画面色彩明亮饱和，动作夸张且富有弹性，背景细节丰富，光影效果梦幻，适合15秒的视频时长。"`
        : `✅ 示例：
...
- 包含所有必要信息（风格、角色、场景、动画、时长）`;
    ...
    const exampleSection = (context.characters && context.characters.length > 0)
      ? `✅ 必须包含角色引用的示例：
"${styleText}的视频。@${context.characters[0].username} 在场景中活动，充满活力和好奇。色彩鲜明，动作流畅，适合10秒的视频时长。"`
      : `✅ 示例：
