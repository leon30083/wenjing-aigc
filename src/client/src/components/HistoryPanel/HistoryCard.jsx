 ...
 */
function HistoryCard({ record, onClick, onContextMenu }) {
  const { prompt, status, createdAt, thumbnail, favorite, type, platform, result, model, options } = record;
      ...
      {(model || options) && (
        <div style={{
          ...
          {options?.watermark !== undefined && (
            <div style={{ marginBottom: '2px' }}>
              <strong>水印:</strong> {options.watermark ? '开启' : '关闭'}
            </div>
          )}
          {result?.output && (
            <div style={{
