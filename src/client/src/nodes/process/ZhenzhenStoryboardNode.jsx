...
function ZhenzhenStoryboardNode({ data }) {
  ...
  const [apiConfig, setApiConfig] = useState(() => {
    ...
    return {
      platform: 'zhenzhen', // 固定为贞贞
      model: 'sora-2', // 默认使用 sora-2（可选 sora-2-pro）
      aspect: '16:9',
      hd: false, // 贞贞特有：高清选项
      watermark: false,
      apiKey: savedApiKey,
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        window.dispatchEvent(new CustomEvent('video-task-created', {
          detail: { sourceNodeId: nodeId, taskId: id, platform: 'zhenzhen' }
        }));
      } else {
        setStatus('error');
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'absolute', right: '18px', top: '50%', transform: 'translateY(-50%)', zIndex: 10 }}>
        <span style={{ fontSize: '10px', color: '#f59e0b', fontWeight: 'bold', whiteSpace: 'nowrap' }}>视频</span>
      </div>
        ...
        {showApiConfig && (
          <div style={{
            ...
            <div style={{ marginBottom: '6px' }}>
              <label style={{ display: 'block', marginBottom: '2px', fontWeight: 'bold', color: '#92400e' }}>
                平台:
              </label>
              <input
                type="text"
                value="贞贞 (ai.t8star.cn)"
                disabled
                style={{
                  width: '100%',
                  padding: '4px',
            ...
            <div style={{ marginBottom: '6px' }}>
              <label className="nodrag" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                <input
                  className="nodrag"
                  type="checkbox"
                  checked={apiConfig.hd}
                  onChange={(e) => setApiConfig({ ...apiConfig, hd: e.target.checked })}
                  style={{ cursor: 'pointer' }}
                />
                <span style={{ fontWeight: 'bold', color: '#92400e' }}>高清模式 (HD)</span>
              </label>
            </div>
            ...
            <div style={{ marginBottom: '6px' }}>
              <label className="nodrag" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                <input
                  ...
                  style={{ cursor: 'pointer' }}
                />
                <span style={{ fontWeight: 'bold', color: '#92400e' }}>启用水印</span>
              </label>
            </div>
      ...
      {connectedImages.length > 0 && (
        <div className="nodrag" style={{ padding: '6px', backgroundColor: '#fef3c7', borderRadius: '4px', marginBottom: '8px' }}>
          <div style={{ marginBottom: '4px', fontWeight: 'bold', color: '#92400e' }}>
          ...
          <label className="nodrag" style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '6px' }}>
            <input
              ...
              style={{ cursor: 'pointer' }}
            />
            <span style={{ fontSize: '11px', color: '#92400e', cursor: 'pointer' }}>
              启用全局参考图
            </span>
          </label>
          ...
          <div style={{ display: 'flex', gap: '4px' }}>
            {connectedImages.map((url, index) => (
              <img
                ...
                src={url}
                alt={`global-${index}`}
                style={{ width: '36px', height: '36px', borderRadius: '3px', border: '1px solid #fcd34d' }}
              />
            ))}
          </div>
        </div>
        ...
        {shots.map((shot, index) => (
          ...
          >
            <div style={{ display: 'flex', gap: '4px', alignItems: 'center', marginBottom: '4px' }}>
              ...
              />
              <span style={{ fontSize: '9px', color: '#92400e' }}>秒</span>
              <button
                className="nodrag"
                onClick={() => {
...
export default ZhenzhenStoryboardNode;
