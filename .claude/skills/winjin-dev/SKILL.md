...
### 错误47: 图片加载导致布局抖动（CLS） ⭐ 2026-01-04 新增
...
- **根本原因**: 图片加载时没有预留空间，导致布局频繁调整
  - 每个角色库节点有 15 张图片
  - 每张图片加载时触发微小布局偏移（0.0001-0.0005 分数）
  - 多个节点累积导致 CLS（累积布局偏移）分数达到 0.0176
  - 持续约 5 秒，看起来像"滚动条在滑动"
- **错误尝试**:
  - ❌ 使用 `useMemo` 优化 filteredCharacters - 无效
  - ❌ 优化 `onSizeChange` 赋值 - 无效
  - ❌ 添加 `React.memo` 包装组件 - 无效
  - ❌ 检查控制台日志 - 无无限循环
- **正确做法**: 使用浏览器性能录制定位问题，然后添加 CSS 属性防止布局偏移
  ```javascript
  // ❌ 错误：图片没有约束布局
  <img
...
- **关键点**:
  1. **display: 'block'**: 防止内联布局计算，图片加载前预留空间
  2. **flexShrink: 0**: 防止 flex 容器收缩导致图片尺寸变化
  3. **default-avatar.svg**: 必须存在，否则图片加载失败会持续重试
  4. **CLS 分数阈值**: 良好 ≤ 0.1，需要改进 0.1-0.25，糟糕 > 0.25
  5. **性能录制是关键**: 比查看控制台日志更有效，能直接定位布局问题
- **验证结果**: ✅ CLS 从 0.0176 降到 0.0010，节点内容不再抖动
- **修复文件**:
  - `src/client/src/nodes/input/CharacterLibraryNode.jsx` - Lines 715-716（添加 CSS 属性）
  - `src/client/public/default-avatar.svg` - 新建默认头像文件
- **修复日期**: 2026-01-04
- **经验教训**:
...
### 🔥 调试流程优化（2026-01-04 更新）

**优先级 1: 性能问题诊断 ⭐ 黄金方法**

当遇到 UI 抖动、卡顿、布局问题时，**优先使用浏览器性能录制功能**：

```bash
# 性能录制流程（Chrome DevTools）
1. 打开 Chrome DevTools（F12）
2. 切换到 Performance 选项卡
3. 点击录制按钮（圆点图标）
4. 刷新页面（Ctrl+Shift+R 强制刷新，清除缓存）
5. 等待 5-10 秒（覆盖问题发生的时间窗口）
6. 停止录制
7. 查看关键指标：
...
**为什么性能录制优先**：
- ✅ 直观：看到实际的性能数据和问题
- ✅ 精准：定位到具体的问题类型（布局偏移 vs 渲染阻塞 vs JS 执行）
- ✅ 快速：5 分钟内找到根本原因（vs 数小时误诊）
- ✅ 全面：同时检查多个性能指标

**性能录制发现的典型问题**：
- **布局偏移（CLS）**: 图片/广告加载导致内容跳动
- **长任务（Long Tasks）**: JavaScript 执行超过 50ms
- **渲染阻塞**: 大量 DOM 操作导致卡顿
- **网络慢**: 资源加载时间过长
