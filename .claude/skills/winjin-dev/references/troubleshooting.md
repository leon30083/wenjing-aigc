...
3. [性能与布局问题](#性能与布局问题) ⭐ 新增
4. [Sora2 平台问题](#sora2-平台问题)
5. [角色系统问题](#角色系统问题)
6. [文档更新问题](#文档更新问题)
...
## 性能与布局问题 ⭐ 2026-01-04 新增

### 问题 7: UI 抖动/卡顿/内容跳动 ⭐ 黄金调试方法
...
3. ❌ 怀疑是实际滚动 → 添加标记线，证明滚动条静止
4. ✅ 使用浏览器性能录制功能 → 发现 51 次布局偏移
...
5. 等待 5-10 秒（覆盖问题发生时间）
6. 停止录制
7. 查看关键指标：
   - CLS (Cumulative Layout Shift) - 布局偏移分数
   - 偏移次数和持续时间
8. 点击 "Layout shifts" 查看详细信息
```

**根本原因**:
- 图片加载时没有预留空间
- 每张图片触发微小布局偏移（0.0001-0.0005 分数）
- 多个节点累积导致 CLS 分数过高
...
3. **default-avatar.svg**: 必须存在，否则持续重试导致闪动
...
| 优先级 | 方法 | 适用场景 | 用时 |
|--------|------|----------|------|
| ⭐ 1 | 性能录制 | UI 抖动、卡顿、布局问题 | 5 分钟 |
| 2 | 控制台日志 | 无限循环、错误日志 | 10-30 分钟 |
| 3 | 网络请求 | API 失败、数据格式错误 | 10 分钟 |
| 4 | 断点调试 | 变量值、调用栈 | 15-30 分钟 |

**相关错误**: 错误47 - 图片加载导致布局抖动（CLS）
