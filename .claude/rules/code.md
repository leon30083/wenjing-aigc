...
useEffect(() => {
  // ⚠️ 问题：这个检查在 _isCompletedFromHistory 之前
...
**问题**:
1. **检查顺序错误**: ref 检查在 `_isCompletedFromHistory` 标志之前执行
2. **历史记录加载被拦截**: `_isCompletedFromHistory: true` 时，如果 ref 是 `false`，会直接 return
3. **状态未恢复**: TaskResultNode 显示错误视频或状态未正确恢复

**根本原因**:
- 历史记录加载时设置 `data._isCompletedFromHistory = true`
- useEffect 1 执行，但 `isCompletedFromHistoryRef.current` 可能是 `false`（新任务）
- ref 检查优先执行，直接 return，跳过了 `_isCompletedFromHistory` 检查

**场景**:
```
1. 用户生成新视频（isCompletedFromHistoryRef.current = false）
2. 用户加载历史记录（data._isCompletedFromHistory = true）
3. useEffect 1 执行，但 ref 检查优先，直接 return ❌
4. TaskResultNode 状态未恢复，显示错误数据 ❌
```

**关键点**:
1. **检查顺序很重要**: 先检查 `_isCompletedFromHistory` 标志，再检查 ref
2. **ref 检查的目的**: 防止新任务被历史记录覆盖
3. **历史记录加载**: 应该**总是**恢复状态，不受 ref 影响
4. **标志位优先**: `_isCompletedFromHistory` 是明确的标志，应该优先检查
